...
# Chargement du filtre MIDI
try:
    from filter_processor import get_filter_processor
    _use_filter = _get_option(_Config, "enableMidiFilter", True)
    if _use_filter:
        _midi_filter = get_filter_processor()
    else:
        _midi_filter = None
except ImportError:
    _midi_filter = None
...
if not _get_option(_Config, "exploreMode"):
    ...
    # Injection du filtre MIDI
    if _midi_filter:
        _midi._filter = _midi_filter
